...
echo "Inicio del script de recuperación a las: $(date)"
...
# Ejecuta rsync para hacer el respaldo en la Raspberry Pi
 rsync -avr --progress   ${USUARIO}@${IP_RASPBERRY}:/mnt/unidadProyecto/${ESPACIO}/   ${RUTA_LOCAL} 
...
echo "Fin del script de recuperación a las: $(date)"
